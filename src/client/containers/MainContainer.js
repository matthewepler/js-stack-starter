...
class MainContainer extends Component {
  constructor (props) {
    ...
    this.activeThreshold = 3000 // how long before a tag is 'greyed' out
    this.heartbeatThreshold = 60000 // how long before a tag is removed
  ...
  render () {
    ...
    if (this.state.tagId.length > 0) {
      let matchedTags = this.state.activeTags.filter((tag) => {
        console.log(`attempting match: ${tag.id} : ${this.state.tagId}`)
        return tag.id.includes(this.state.tagId)
      })
      console.log('returned matches = ', matchedTags.length)
      if (matchedTags) {
        tags = matchedTags.map((tag, index) => {
          return (
            <TagRow
              key={index}
              id={tag.id}
    ...
    return (
      <div className='main-container'>
        <div className='nav-bar' >
          <img
            src='static/img/dark_hexagon.svg'
            alt='refresh'
